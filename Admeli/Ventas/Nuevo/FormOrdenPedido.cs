...
{
    public partial class FormOrdenPedido : Form
    {
        ...
        private List<DetalleOrden> detalleA = new List<DetalleOrden>();
        ...
        List<DetalleOrden> detalleOrdenResumen= new List<DetalleOrden>();
        ...
        public FormOrdenPedido(OrdenCompra currentOrdenCompra)
        {
            InitializeComponent();   
        ...
        public FormOrdenPedido(List<DetalleV> detallePedidoPRoveedor, Proveedor proveedor)
        {
            InitializeComponent();
            this.detallePedidoPRoveedor = detallePedidoPRoveedor;

            this.idMoneda = detallePedidoPRoveedor[0].idMoneda;
        ...
        public FormOrdenPedido(List<DetalleV> detallePedidoPRoveedor, OrdenCompra currentOrdenCompra, Proveedor proveedor)
        {
            InitializeComponent();
            this.detallePedidoPRoveedor = detallePedidoPRoveedor;
            this.proveedor = proveedor;
        ...
        private  async void cargarOrdenResumen(List<DetalleV> detallePedidoPRoveedor)
        {
            ...
            try
            {
                ...
                foreach (DetalleV DV in detallePedido)
                {
                    ...
                    detalleOrden.descuento = (double)DV.descuento;
                    ...
                    detalleOrden.cantidadUnitaria = cantidad;
                    detalleOrden.precioUnitario = precioUnitairo;
                    detalleOrden.total = cantidad * precioUnitairo;
                ...
                if(detalleA.Count==0)
                {
                    MessageBox.Show("No hay stock suficiente del proveedor para hacer la Orden. Comuniquese con su proveedor ", "Pedido", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    this.Close();
                }
                List<DetalleOrden> detalleOrdenV = new List<DetalleOrden>();
                ...
                foreach (DetalleV DV in detallePedido)
                {
                    ...
                    detalleOrden.precioUnitario = precioUnitairo;
                    detalleOrden.total = cvv * precioUnitairo;
                    detalleOrden.idPedido = DV.idDetalleCotizacion;
                    // agrgando un nuevo item a la lista
                    DetalleOrden find = detalleOrdenV.Find(X => X.idPresentacion == DV.idPresentacion && X.idCombinacionAlternativa == DV.idCombinacionAlternativa);
                    if (find == null && cvv>0)
                    {
                        detalleOrdenV.Add(detalleOrden);
                    }
                }

                detalleOrdenBindingSource.DataSource = detalleOrdenV;
        ...
        private void reLoad()
        {
            ...
            cargarProductos();                   
            cargarPresentacion();
            cargarOrdenResumen(detallePedidoPRoveedor);
        }
        #endregion
        #region ======================== KEYBOARD ========================
        ...
        private async void cargarMonedas()
        {
            try
            {
                ...
                if (this.idMoneda != 0)
                {
                    cbxTipoMoneda.SelectedValue = idMoneda;
                }
            }
            catch (Exception ex)
            {
        ...
        private async void hacerOrdenCompra()
        {
            if (nroNuevo != 1)
            {
                ...
                {
                    //validar 
                    MessageBox.Show("No hay ningún proveedor seleccionado", "Proveedor", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    txtRuc.Focus();
                    return;
                }
                if(detalleA==null || detalleA.Count == 0)
                {
                    MessageBox.Show("No hay ningún productos seleccionados seleccionado", "Productos", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    cbxCodigoProducto.Focus();
                    return;    
                }
                ...
                try
                {
                    response=  await ordenCompraModel.guardarOrdenPedido(compraTotal);

                    MessageBox.Show(response.msj, "Guardar", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    if (response.id > 0)
                    {

                        currentOrdenCompra = new OrdenCompra();
                        currentOrdenCompra.idOrdenCompra = response.id;
                    }
                }
                catch(Exception ex)
                {
        ...
        private  async void btnImportar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DetalleV DV in detallePedido)
                {
                    ...
                    detalleOrden.nombrePresentacion = DV.nombrePresentacion;
                    detalleOrden.nro = 1;
                    detalleOrden.precioUnitario =(double) DV.precioUnitario;
                    detalleOrden.total = toDouble(DV.cantidad)*  (double)DV.precioUnitario;
                    detalleOrden.idPedido = DV.idDetalleCotizacion;
                    // agrgando un nuevo item a la lista
                    detalleA.Add(detalleOrden);
                }
