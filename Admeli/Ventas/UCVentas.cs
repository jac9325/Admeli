...
namespace Admeli.Ventas
{
    public partial class UCVentas : UserControl
    {
        ...
        {
            dataGridView.Columns["total"].DefaultCellStyle.Format = ConfigModel.configuracionGeneral.formatoDecimales;
        ...
        private async void cargarPersonales()
        {
            try
            {
                ...
                if (ConfigModel.asignacionPersonal.idPuntoGerencia != 0 || ConfigModel.asignacionPersonal.idPuntoAdministracion != 0)
                {
                    ...
                    dataGridView.Columns["vendedor"].Visible = true;
                }
                else
                {
        ...
        private async void cargarPuntosVenta( int idSucursal)
        {
            try
            {
                ...
                listaPuntoVenta.AddRange(ConfigModel.puntosDeVentaTodos);
        ...
        private async void cargarRegistrosLike()
        {
            ...
            try
            {
                ...
                RootObject<Venta> rootData = await ventaModel.ventaslike(sucursalId, puntoVentaId, personalId, estado, paginacion.currentPage, paginacion.speed,nombre);
                // actualizando datos de páginacón
                paginacion.itemsCount = rootData.nro_registros;
                paginacion.reload();
                // Ingresando
        ...
        #region Modificar Procedimiento Almacenado
        private  string Modificar()
        {
            ...
            try
            {
                sqlcon.ConnectionString = "Data Source=PC;Initial Catalog = fact_nuevo2;Integrated Security=true";
                sqlcon.Open();

                SqlCommand SqlCmd = new SqlCommand();
                SqlCmd.Connection = sqlcon;
                SqlCmd.CommandText = "sp_modificar_venta";
                ...
                rpta = SqlCmd.ExecuteNonQuery() == 1 ? "OK" : "izi modificacion";
            }
            catch (Exception ex)
            {
        ...
        public static void modif(int id_venta, string oservacion)
        {
            
        }

        private async void EjecutarModificar()
        {
            string rpta = "";
            rpta = Modificar();
        }
        ...
        private  async void cbxSucursales_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                List<PuntoDeVenta> lista = ConfigModel.puntosDeVentaTodos.Where(X => X.idSucursal == (int)cbxSucursales.SelectedValue).ToList();
