    ...
    public partial class UCCierreCaja : UserControl
    {
        ...
        private async void cargarSucursales()
        {
            try
            {
                ...
                List<Sucursal> listaSucursalesCargar = new List<Sucursal>();
                List<Sucursal> listaSucursales = ConfigModel.listSucursales;
                Sucursal sucursal = new Sucursal();
                sucursal.idSucursal = 0;
                sucursal.nombre = "Todas las sucursales";
                listaSucursalesCargar.Add(sucursal);
                listaSucursalesCargar.AddRange(listaSucursales);
                sucursalBindingSource.DataSource = listaSucursalesCargar;
        ...
        private async void cargarPersonales()
        {
            try
            {
                if (ConfigModel.asignacionPersonal.idPuntoGerencia == 0 && ConfigModel.asignacionPersonal.idPuntoAdministracion == 0)
                {
                    ...
                    dataGridView.Columns["nombres"].Visible = false;
                }
                else
                {
                    personalBindingSource.DataSource = await personalModel.listarPersonalCaja(0);
                    cbxPersonales.SelectedValue = PersonalModel.personal.idPersonal;
                    dataGridView.Columns["nombres"].Visible = true;
                }
            }
            catch (Exception ex)
            {
        ...
        private async void cbxSucursales_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    personalBindingSource.DataSource = await personalModel.listarPersonalCaja((int)cbxSucursales.SelectedValue);
                    cbxPersonales.SelectedValue = PersonalModel.personal.idPersonal;
                }
            }
            catch (Exception ex)
        ...
        private void lblPageCount_KeyUp(object sender, KeyEventArgs e)
        {
            if(e.KeyCode==Keys.Enter)
            {               
                cargarRegistros();              
