...
namespace Admeli.Herramientas
{
    public partial class UCInicializarStock : UserControl
    {
        ...
        private void darFormatoDecimales()
        {
            ...
            dataGridView.Columns["precioConImpuesto"].DefaultCellStyle.Format = ConfigModel.configuracionGeneral.formatoDecimales;
            dataGridView.Columns["utilidad"].DefaultCellStyle.Format = ConfigModel.configuracionGeneral.formatoDecimales;
            ...
            dataGridView.Columns["precioConImpuesto"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dataGridView.Columns["utilidad"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
        ...
        public async void cargarMonedas()
        {
            loadState(true);
            try
            {
               ...
               Moneda moneda = monedas.Find(X => X.porDefecto);
                lbMoneda.Text = "Precio en: "+moneda.moneda; 
        ...
        private async void cargarRegistros()
        {
            ...
            try
            {
                ...
                if (sendList.Count > 0)
                {
                    ...
                    dataGridView.DataSource=productoDataBindingSource;
        ...
        private void dataGridView_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            int idProducto = Convert.ToInt32(dataGridView.CurrentRow.Cells["idProducto"].Value); // obteniedo el idCategoria del datagridview
            int idAlmacen = Convert.ToInt32(dataGridView.CurrentRow.Cells["idAlmacen"].Value); // obteniedo el idCategoria del datagridview
        ...
        private void executeModificar() // no cargar el form de producto
        {
            ...
            int index = dataGridView.CurrentRow.Index; // Identificando la fila actual del datagridview
            int idPresentacion = Convert.ToInt32(dataGridView.Rows[index].Cells["idPresentacion"].Value); // obteniedo el idRegistro del datagridview
        ...
        private async void btnGuardar_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView.Rows)
                {
                    decimal pcompra = (decimal)row.Cells["precioCompra"].Value;
                    decimal pcompraImpuesto = (decimal)row.Cells["precioConImpuesto"].Value;
                    decimal pVenta = (decimal)row.Cells["precioVenta"].Value;
                    decimal stock = (decimal)row.Cells["stock"].Value;
                    int idPresentacion= (int)row.Cells["idPresentacion"].Value;//idAlmacen
                    int idAlmacen = (int)row.Cells["idAlmacen"].Value;
                    ProductoData auxProducto = productos.Find(X => X.idPresentacion == idPresentacion && X.idAlmacen == idAlmacen);
                    auxProducto.precioCompra = pcompra;
                    auxProducto.precioConImpuesto = pcompraImpuesto;
                    auxProducto.precioVenta = pVenta;
        ...
        private void dataGridview_text(object sender, EventArgs e)
        {
            ...
            int idpresentacion = (int)dataGridView.Rows[index].Cells["idPresentacion"].Value;
        ...
        private void dataGridView_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if(e.Button == MouseButtons.Right)
            {
                ...
                int idPresentacion = Convert.ToInt32(dataGridView.Rows[index].Cells["idPresentacion"].Value);
            ...
            switch (name)
            {
                ...
                case "idPresentacion":
                    ...
                    if (columnSort[name])
                    {
                        ...
                        dataGridView.DataSource = productos.OrderBy(s => s.idPresentacion).ToList();

                    }
                    else
                    {
                        columnSort[name] = true;
                        dataGridView.DataSource = productos.OrderByDescending(s => s.idPresentacion).ToList();
                ...
                case "precioCompra":
                    ...
                    break;
                case "precioConImpuesto":

                    if (columnSort[name])
                    {
                        columnSort[name] = false;
                        dataGridView.DataSource = productos.OrderBy(s => s.precioConImpuesto).ToList();

                    }
                    else
                    {
                        columnSort[name] = true;
                        dataGridView.DataSource = productos.OrderByDescending(s => s.precioConImpuesto).ToList();
                ...
                case "utilidad":
                    ...
                    {
                        columnSort[name] = false;
                        dataGridView.DataSource = productos.OrderBy(s => s.utilidad).ToList();

                    }
                    else
                    {
                        columnSort[name] = true;
                        dataGridView.DataSource = productos.OrderByDescending(s => s.utilidad).ToList();
                ...
                case "almacen":
                    ...
                    {
                        columnSort[name] = false;
                        dataGridView.DataSource = productos.OrderBy(s => s.almacen).ToList();

                    }
                    else
                    {
                        columnSort[name] = true;
                        dataGridView.DataSource = productos.OrderByDescending(s => s.almacen).ToList();
